...
| 平台          | 常用选项                                                                      |
| ----------- | ------------------------------------------------------------------------- |
| Linux/macOS | `-c 次数`、`-i 间隔`、`-s 包大小`、`-W 超时`、`-t TTL`、`-I 接口`、`-4/-6`、`-q 简洁`、`-f 洪水` |
| Windows     | `-t 持续`、`-n 次数`、`-l 包大小`、`-f 不分片`、`-i TTL`、`-w 超时`、`-4/-6`、`-a 解析主机名`     |

查看全部：Linux/macOS 用 `man ping`，Windows 用 `ping /?`。
#### 5、示例
...
关键点：
- 发的是 ICMP Echo Request，Type=8。
- 收的是 ICMP Echo Reply，Type=0。
- 靠 Identifier + Sequence 匹配请求与回复。
- TTL 在 IP 头里，不在 ICMP 里。

回显包示例

```text
08 00 33 37 00 01 00 01 61 62 63 64
```
...
00 01     Identifier
00 01     Sequence
61 62 63 64  Data = "abcd"
```

```text
00 00 3B 37 00 01 00 01 61 62 63 64
```
...
实际 ping 的 Data 通常包含时间戳，长度也更长，所以校验和会不同。