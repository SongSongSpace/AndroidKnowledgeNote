#网络通信 

**目标**：让两个都在 NAT/防火墙后面的设备，建立一条**直接 P2P 连接**，而不是所有数据都走公网服务器中转。

### 核心原理
1. A、B 先连接一个公网信令服务器。
2. 服务器告诉 A：B 的公网地址是 `1.2.3.4:5000`；告诉 B：A 的公网地址是 `5.6.7.8:6000`。
3. A、B **同时向对方的公网地址发包**。
4. 这些包会在各自 NAT 上创建映射/放行规则。
5. 之后双方就能直接通信，服务器只负责协调。

### 常见相关协议/技术
- STUN：发现自己的公网 IP:Port，判断 NAT 类型。RFC 8489。
- TURN：打洞失败时，用公网服务器中继流量。RFC 8656。
- ICE：综合 STUN/TURN，收集候选地址并做连通性检查。WebRTC 核心。RFC 8445。
- UDP 打洞：最常用，成功率高。
- TCP 打洞：利用 TCP 同时打开，成功率较低，受系统和 NAT 影响。
- UPnP IGD / NAT-PMP / PCP：不是严格打洞，而是向网关请求端口映射。PCP RFC 6887，NAT-PMP RFC 6886。
- Tailscale / ZeroTier / WebRTC：都包含打洞 + 中继回退机制。

### 成功条件与限制
- 锥形 NAT 成功率较高。
- 对称 NAT、CGNAT、企业防火墙下经常失败，需要 TURN 之类中继。
- 打洞本身不提供加密，通常还要叠加 TLS、DTLS、WireGuard 等。
- 需要保活包，否则 NAT 映射会过期。

### 典型应用
WebRTC 视频通话、游戏联机、P2P 下载、远程桌面、Mesh VPN、IoT 直连等。

### 伪代码

```python
sock = UDP socket
sock.bind(("0.0.0.0", 0))          # 本地端口
A_pub = stun_get_mapped(sock)       # 问 STUN 自己的公网地址
send_to_signal(A_pub)               # 告诉信令服务器
B_pub = recv_from_signal()          # 拿到对方公网地址

for i in range(100):
    sock.sendto(b"PING", B_pub)
    try:
        data, addr = sock.recvfrom(1024)
        if data == b"PING":
            sock.sendto(b"PONG", addr)
        elif data == b"PONG":
            print("打洞成功")
            break
    except BlockingIOError:
        pass
    sleep(0.02)
```

### QA

1、信令服务器怎么知道要交换？

	本身不知道，是客户端主动注册到同一个会话/房间。关键是：**双方事先约定同一个房间号/会话 ID，或者由一方创建会话、另一方加入。**

2、双方都主动发 + 持续重试 + 同一个端口收发。重试多久会放弃。

	没有统一标准，取决于具体实现。
	
	常见策略：
	- 每 `20ms ~ 50ms` 发一次。
	- 持续 `5 ~ 10 秒`。
	- 或者指数退避：`100ms, 200ms, 400ms, 800ms...`
	- WebRTC ICE 的连通性检查总超时常见在 `10 ~ 30 秒` 左右。
	- 如果失败，就回退到 **TURN 中继**。
	
	应用可以自己自定义。
	
	另外，UDP NAT 映射有超时，通常 `30 秒 ~ 几分钟`。  
	所以打洞成功后，还要定期发 **keepalive** 包，否则洞会关掉。

3、UDP socket 建立需要什么条件？什么时候建立？什么时候释放？

	UDP 是无连接协议。
	
	建立条件。内核分配一个 socket 文件描述符和本地端口资源。可以 bind() 指定本地 IP:端口，也可以不 bind。
	```c
	int fd = socket(AF_INET, SOCK_DGRAM, 0);
	```
	 什么时候建立？
	- 程序调用 `socket()` 时创建。
	- 调用 `bind()` 时绑定本地地址/端口。
	- 第一次 `sendto()` 时，如果没有 bind，内核自动绑定临时端口。
	- 没有“连接建立”握手。
	
	什么时候释放？
	- 程序调用 `close(fd)`。
	- 进程退出，内核回收所有 fd。
	- 程序崩溃，内核也会回收。
	
	但注意：  
	**UDP socket 关闭 ≠ NAT 映射立刻消失。**  
	NAT 映射有自己的超时时间，可能还会保留几十秒到几分钟。

4、TURN 中继会失败么？失败后的下一步？

	会失败。
	
	原因：
	- 客户端连不上 TURN 服务器：防火墙封了 UDP/TCP 端口。
	- TURN 服务器宕机、过载、带宽满。
	- 认证失败：用户名、密码、token 错误。
	- 端口耗尽：TURN 服务器没有可用中继端口。
	- 企业网络只允许 443/HTTPS，而 TURN 用的端口被封。
	- TURN 服务器本身没有公网可达地址，或者被墙。
	
	失败后下一步：
	- 换其它 TURN 服务器：ICE 会收集多个候选。
	- 换传输协议：UDP -> TCP -> TLS -> TLS over 443。
	- 换端口：3478、5349、443等。
	- 降级到应用层中继：
		- WebSocket 中继
		- HTTP 长轮训
		- 自建反向代理
		- 云函数/消息队列转发
	- 最终失败：提示用户 无法建连，或只能通过服务器转发消息，不能实时 P2P。
	即：失败后，不是立马没路，而是继续降级找到更可靠但更慢的通道。

